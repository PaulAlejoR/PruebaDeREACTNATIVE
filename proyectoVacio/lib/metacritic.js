export async function getLastesGames() {
    const LATEST_GAMES =
...
export async function getNameDetails(slug) {
    ...
    // get the reviews
    const reviews = rawReviews.map(( reviews ) => {
        const { quote, score, date, publications, author } = reviews;
        return { quote, score, date, publications, author };
    });
